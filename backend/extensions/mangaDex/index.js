  ...
  return (
    ...
    (params
      ? "?" +
        Object.keys(params)
          .map((x) =>
            Array.isArray(params[x])
              ? params[x].map((y) => `${x}[]=${y}`).join("&")
              : `${x}=${JSON.stringify(params[x])}`,
          )
          .join("&")
      : "")
  );
...
async function search(search, page) {
  ...
  if (!search) {
    data = await (
      await fetch(
        `https://api.mangadex.org/manga?limit=${numberOfComics}&offset=${offset}&includes%5B%5D=cover_art&order[rating]=desc&contentRating%5B%5D=safe`,
      )
    ).json();
  } else {
    data = await (
      await fetch(
        `https://api.mangadex.org/manga?limit=${numberOfComics}&offset=${offset}&title=${encodeURI(search)}&includes%5B%5D=cover_art&order[rating]=desc&contentRating%5B%5D=safe`,
      )
    ).json();
  }
...
async function getInfo(url) {
  ...
  let f = fetch(
    "https://api.mangadex.org/manga/" +
      mangaId +
      "?includes%5B%5D=cover_art&includes%5B%5D=author&includes%5B%5D=artist&includes%5B%5D=creator",
  );
...
async function getChapData(url) {
  //url of a chapter
  var imageArr = [];
  var chapId = url.slice(29);
  let data = await (
    await fetch(`https://api.mangadex.org/at-home/server/${chapId}`)
  ).json();
  let chap = data.chapter.data;
  Object.keys(chap).forEach((key) => {
    imageArr.push(
      `https://cmdxd98sb0x3yprd.mangadex.network/data/${data.chapter.hash}/${chap[key]}`,
    );
  });
