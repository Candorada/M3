...
const DownloadButton = React.forwardRef(({ chapter }, ref) => {
  ...
  function Loading({ progress, total, queued = true }) {
    ...
    return (
      <div className="loading" onClick={del}>
        <div
          className="variableSize"
          style={{ "--var": prog * 100 + "%" }}
        ></div>
  ...
  return (
    <div className="downloadBTN">
      {progress != undefined && (progress < total || progress == 0) ? (
        <Loading progress={progress} total={total} queued={queued} />
      ) : (
        btn
      )}
    </div>
...
function MutliChapters({ item, childRefs }) {
  ...
  function read(read = true) {
    let ids = [...document.querySelector("select").selectedOptions].map(
      (x) => x.value,
    );
    fetch("http://localhost:3000/read", {
      ...
      if (x.ok) {
        ids.forEach((x) => {
          document
            .querySelector('[chapter_id = "' + x + '"]')
            .classList.toggle("read", read);
        });
      }
    });
  }
        ...
        <div className="content">
          ...
          <div className="ChapterControlButtons">
            ...
            <input
              ...
              className="multiReadBTN"
              onClick={() => read(true)}
            />
            <input
              type="button"
              value="Mark Selected As Unread"
              className="multiUnreadBTN"
              onClick={() => read(false)}
            />
          </div>
        </div>
...
function MainItemPage({ item }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/${item.extension}/addToLibrary`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: item.source, extension: item.extension }),
    }).then(async (x) => {
      let json = await x.json();
    });
    let interval = setInterval(() => {
      let activeDownloadStringified = JSON.stringify(activeDownloads);
      fetch("http://localhost:3000/downloadingMedia", { cache: "no-cache" })
        .then((res) => res.json())
        .then((json) => {
...
export default function ItemPage() {
  ...
  useEffect(() => {
    try {
      if (
        item.extension &&
        extensions?.[item.extension]?.properties?.customItemPage
      ) {
        let pagePath = extensions?.[item.extension]?.properties?.customItemPage;
        pagePath = pagePath.toLowerCase().endsWith(".jsx")
          ? pagePath.substring(0, pagePath.length - 4)
          : pagePath;
        import(`../backend/extensions/${item.extension}/${pagePath}.jsx`).then(
          async (x) => {
  ...
  return extensions?.[item.extension]?.properties?.customItemPage ? (
    customPage
  ) : extensions && item.cover != defaultItem.cover && item != undefined ? (
    <MainItemPage item={item} />
  ) : (
    <></>
  );
}
